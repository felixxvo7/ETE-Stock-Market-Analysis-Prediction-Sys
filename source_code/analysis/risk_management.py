...
df['ATR'] = df.groupby('Symbol')['Close'].transform(lambda x: x.rolling(14).std())
...
df['Position_Size'] = risk_per_trade / (STOP_LOSS_MULTIPLIER * df['ATR'])
...
df['Weight'] = df['Position_Size'] / df['Position_Size'].sum()
df = df[df['Weight'] > 0]
